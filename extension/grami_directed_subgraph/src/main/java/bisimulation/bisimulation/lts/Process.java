package bisimulation.bisimulation.lts;
...
import Dmine.Rule;
import bisimulation.bisimulation.util.StringUtil;
...
public class Process {
	...
	public static Process parse(final Rule rule, final String processName, final String prefix) throws IOException {
		Process p = new Process(processName+rule.getPattern().hashCode());

        for (Map.Entry<Integer, List<MyPair<String, Integer>>> entry : rule.getProcessMap().entrySet()) {
            p.addState(prefix + entry.getKey());
            for (MyPair pair : entry.getValue()) {
                p.addState(prefix + (Integer) pair.getB());
                p.addAction((String) pair.getA());
                p.addTransition(prefix + entry.getKey(), (String) pair.getA(), prefix + (Integer) pair.getB());
            }
        }
