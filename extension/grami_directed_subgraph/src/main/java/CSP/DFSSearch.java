...
//    }
	...
	/**This function adapts GraMi to output all of the graph embeddings which follow the Minimum image based metrics
...
//        }
	...
	/**
	 *@discription: 重载searchExistances方法，规定目标节点的id
	 *@param target_node_id
	 *@date: 2020/3/15 12:21
	 *@return: void
	 *@author: Han
	 */
	public void searchExistances(int target_node_id) {
...
//		for (int i = variables.length-1; i >=0 ; i--) {} //Line 9 in Algorithm ISFREQUENT
// 			System.out.println("\n");
//			System.out.println("now begin to run the target variable, whose target node id is " + target_node_id + ". There are totally " + variables.length + " variables.");//added on 22 Jan 2019
		// 指定target_node对应的variable,进行每一个assignment的search
		int i=0;
		for (int j=0; j < variables.length; j++)
		{
			if (target_node_id == variables[j].getID())
			{
				i = j;
			}
		}
...
//			}
	...
	public List<Integer> getListOfX(int id) {
	    List<Integer> list = null;
	    for (int i = 0; i < result.length; i++) {
	        if (id == result[i].getID()) {
	            HashMap<Integer, myNode> gMap = result[i].getList();
	            list = new ArrayList<>(gMap.size());
	            for (Map.Entry<Integer, myNode> entry:gMap.entrySet()){
	...
	public int getFrequencyOfPattern(int id)
	{
		for (int i = 0; i < result.length; i++) {
			if (id == result[i].getID())
				return result[i].getListSize();
		}
		return -1;
	}
...
//			System.out.println(tempCounter++);
    ...
    public void searchAll(int target_node_id)
    {
        setVariableVisitingOrder(getMaxDegreeVariableIndex()); //set variable visit order
        int index=-1;
        ...
        {
            myNode firstNode= iterator.next();
            if (firstNode.getID() == target_node_id) {
                instance.assign(firstVB.getID(), firstNode);
                Node nodeInPattern = new InternalNode(Integer.toString(firstVB.getID()),new Label(Integer.toString(firstVB.getLabel())));//added on 27 Jan 2019
                Node nodeInData = new InternalNode(Integer.toString(firstNode.getID()),new Label(Integer.toString(firstNode.getLabel())));//added on 27 Jan 2019
                instance.addIsomorphism(nodeInPattern, nodeInData);//record the correspondence...important!
